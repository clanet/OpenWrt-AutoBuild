...
sed -i "s/OpenWrt /TT build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ              # 增加自己个性名称
...
cat >> $ZZZ <<-EOF
# 设置旁路由模式
uci set network.lan.gateway='192.168.2.1'                     # 旁路由设置 IPv4 网关
...
# 常用LuCI插件:
cat >> .config <<EOF
CONFIG_PACKAGE_luci-app-oaf=y # appfilter
